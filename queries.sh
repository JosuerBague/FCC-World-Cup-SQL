...
echo "$($PSQL "SELECT DISTINCT name FROM games INNER JOIN teams ON winner_id=team_id ORDER BY 1")"
...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON winner_id=team_id WHERE name ILIKE 'co%'")"
